...
async function authenticateConnection(req) {
  try {
    ...
    console.log('🔐 [WebSocket Auth] Token received:', token ? token.substring(0, 30) + '...' : 'none');
    ...
    try {
      ...
      console.log('✅ [WebSocket Auth] JWT verification successful for user:', decoded.userId);
      ...
      if (!userExists) {
        console.error('❌ [WebSocket Auth] User not found in database:', decoded.userId);
        throw new Error('User not found');
      }
      
      console.log('✅ [WebSocket Auth] User verified:', userExists.name);
      return decoded;
      
    } catch (jwtError) {
      console.log('⚠️ [WebSocket Auth] JWT verification failed, trying as direct user ID:', jwtError.message);
      ...
      if (/^\d+$/.test(token) && token.length > 5) {
        console.log('🔄 [WebSocket Auth] Attempting direct user ID authentication:', token);
        ...
        if (userExists) {
          console.log('✅ [WebSocket Auth] Direct user ID authentication successful:', userExists.name);
          return { userId: token };
        } else {
          console.error('❌ [WebSocket Auth] User ID not found in database:', token);
          throw new Error('User not found');
        }
      }
    ...
    console.error('❌ [WebSocket Auth] Authentication failed:', error.message);
    return null;
  }
}
...
async function handleWebSocketConnection(ws, req) {
  ...
  console.log('🔗 [WebSocket] New connection attempt from:', req.headers.host);
  
  // Authenticate connection (now async)
  try {
  ...
  ws.on('message', (message) => {
    ...
    if (typeof message === 'string') {
      ...
      // Handle array of buffers/strings
      messageStr = message.map(m => m instanceof Buffer ? m.toString() : m).join('');
    } else {
      // Fallback: try to convert to string
      console.warn('⚠️ [WebSocket] Unexpected message type, attempting conversion:', typeof message);
      try {
        messageStr = String(message);
      } catch (e) {
        console.error('❌ [WebSocket] Failed to convert message to string:', e);
        return;
      }
    }
    
    handleMessage(ws, messageStr, userId, socketId);
  });
  ...
  ws.on('pong', () => {
    ...
    console.error('❌ [WebSocket] Authentication error:', authError.message);
