    ...
    <div className={`markdown-content prose prose-invert max-w-none ${className}`}>
      <ReactMarkdown
        ...
        components={{
          // 自定义组件样式
          h1: ({ node, ...props }) => (
            <h1 className="text-3xl font-bold text-white mb-6 mt-8 pb-3 border-b border-gray-700" {...props} />
          ),
          h2: ({ node, ...props }) => (
          ...
          code: ({ node, inline, className, children, ...props }: any) => {
            if (inline) {
              return (
                <code
                  className="bg-gray-800 text-retro-cyan px-1.5 py-0.5 rounded text-sm font-mono"
                  {...props}
                >
                  {children}
                </code>
          ...
          pre: ({ node, ...props }) => (
            <pre className="bg-gray-900 border border-gray-700 rounded-lg p-4 overflow-x-auto mb-4" {...props} />
          ),
          ul: ({ node, ...props }) => (
          ...
          blockquote: ({ node, ...props }) => (
            <blockquote
              className="border-l-4 border-retro-purple pl-4 py-2 my-4 bg-gray-800/30 text-gray-300 italic"
              {...props}
            />
          ),
          table: ({ node, ...props }) => (
          ...
          th: ({ node, ...props }) => (
            <th className="bg-gray-800 text-white px-4 py-2 border border-gray-700 font-bold" {...props} />
          ),
          td: ({ node, ...props }) => (
            <td className="bg-gray-900/50 text-gray-300 px-4 py-2 border border-gray-700" {...props} />
          ),
          img: ({ node, ...props }) => (
