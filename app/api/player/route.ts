...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (stepsToSync > 0) {
      if (stepsToSync >= 5000) {
        await LevelingService.addBits(user.id, Math.floor(stepsToSync / 5000), 'walk', 'step_sync_' + Date.now())
      }
    }
    ...
    if (!recentOnlineReward) {
      await LevelingService.addBits(user.id, 5, 'online_time', 'daily_active_' + Date.now())
    }
