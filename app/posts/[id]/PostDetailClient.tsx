    ...
    <div className={`min-h-screen transition-colors duration-500 ${theme === 'dark' ? 'bg-gray-950' : 'bg-slate-50'} relative`}>
        ...
        <div className="max-w-7xl mx-auto px-4 py-2">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-2 shrink-0">
              <button
                onClick={toggleTheme}
                className={`p-1.5 rounded-lg border transition-all cursor-pointer ${theme === 'dark'
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          <div className="flex-1 min-w-0 w-full flex flex-col gap-1">
              ...
              <div className="p-4 sm:p-8 pb-4">
                <header className="flex items-center space-x-4 mb-4">
                  <div className="relative">
                    <UserAvatar userId={post.author.id} userName={post.author.name} userAvatar={post.author.avatar} customAvatar={post.author.customAvatar} size="xl" showStatus={true} />
                ...
                {post.title && (
                  <div className="mb-4">
                    <h1 className={`text-xl sm:text-2xl font-bold leading-tight break-words ${theme === 'dark' ? 'bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent' : 'text-slate-900'
                      }`}>{post.title}</h1>
                  </div>
                )}
                ...
                <footer className={`flex flex-col sm:flex-row sm:items-center justify-between pt-6 border-t mt-8 mb-4 gap-4 ${theme === 'dark' ? 'border-white/5' : 'border-slate-100'
                  ...
                  <div className="flex flex-wrap items-center justify-between w-full sm:w-auto gap-y-3">
                    <div className="flex items-center gap-3 sm:gap-6 w-full sm:w-auto justify-between sm:justify-start">
                      <div className="flex items-center gap-2 sm:gap-6">
                        {/* 浏览数 */}
                        <div className={`flex items-center space-x-2 text-xs sm:text-base ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                          <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl flex items-center justify-center ${theme === 'dark' ? 'bg-gradient-to-br from-cyan-900/30 to-blue-900/30' : 'bg-gradient-to-br from-cyan-100 to-blue-100'}`}>
                            <svg className={`w-3 h-3 sm:w-4 sm:h-4 ${theme === 'dark' ? 'text-cyan-400' : 'text-cyan-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {/* 回复数 */}
                        <div className={`flex items-center space-x-2 text-xs sm:text-base ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                          <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl flex items-center justify-center ${theme === 'dark' ? 'bg-gradient-to-br from-blue-900/30 to-cyan-900/30' : 'bg-gradient-to-br from-blue-100 to-cyan-100'}`}>
                            <svg className={`w-3 h-3 sm:w-4 sm:h-4 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      >
                        <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl flex items-center justify-center ${post.isLiked
                          ...
                          : theme === 'dark'
                            ? 'bg-gradient-to-br from-emerald-900/30 to-teal-900/30 text-emerald-400'
                        ...
                        <span className={`font-medium ${post.isLiked
                    ...
                    {!isAuthor && (
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-1.5 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white rounded-xl shadow-lg shadow-cyan-900/20 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed group cursor-pointer"
                      >
                        ...
                        <div className="text-left">
                          ...
                          <div className="text-[10px] font-bold leading-tight flex items-center gap-1.5">
                            {t.billboard.push}
                            <span className="bg-white/20 px-1.5 rounded-md text-[9px]">{post.promotionCount || 0}</span>
                          </div>
                        </div>
                      </button>
                    )}
            ...
            <section className={`backdrop-blur-md border-x border-b rounded-b-3xl min-h-[500px] flex flex-col transition-all duration-500 ${theme === 'dark' ? 'bg-[#1a1c1e]/40 border-white/5' : 'bg-slate-50/80 border-slate-200'
              ...
              <div className="p-4 sm:p-8 pt-4 flex-1">
                ...
                {repliesPagination.hasNextPage && (
                  <div className="mt-8 flex justify-center">
                    <button onClick={loadMoreReplies} disabled={isLoadingReplies} className="text-[10px] font-mono text-cyan-500/40 hover:text-cyan-400 uppercase tracking-widest transition-colors cursor-pointer">
      ...
      {lightboxOpen && post.imageUrls && post.imageUrls.length > 0 && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center" onClick={closeLightbox}>
          <button onClick={closeLightbox} className="absolute top-4 right-4 p-2 bg-gray-800/80 text-white rounded-lg cursor-pointer">✕</button>
          ...
          {allPostImages.length > 1 && (
            <>
              <button onClick={e => { e.stopPropagation(); prevImage(); }} className="absolute left-4 p-3 bg-gray-800/80 text-white rounded-lg cursor-pointer">←</button>
              <button onClick={e => { e.stopPropagation(); nextImage(); }} className="absolute right-4 p-3 bg-gray-800/80 text-white rounded-lg cursor-pointer">→</button>
            </>
          )}
        </div>
      )}
